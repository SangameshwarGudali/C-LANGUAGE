...
int to_array(struct node* p_list_1, int** pp_arr, int* p_size)
{
    ...
    p_arr = (int*)malloc(*p_size * sizeof(int));
    assert(p_arr != NULL);
...
void test_suite2(void)
{
    ...
    ret = merge_lists(p_list_1, p_list_2, &p_merged_list);
    assert(ret == SUCCESS);
    ...
    int* p_arr = NULL;
    int size = 0;
    ret = to_array(p_list_1, &p_arr, &size);
    assert(ret == SUCCESS);
    ...
    ret = to_list(p_arr, nr_of_element, &p_list_4);
    assert(ret == SUCCESS);

    show_list(p_list_4, "Showing p_list_4");
